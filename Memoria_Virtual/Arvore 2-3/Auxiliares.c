...
void construir_memoria_do_sistema(Arv23Mem **Raiz) {
    int situacao; 
    if (*Raiz == NULL) {
        ...
        while (Informacao.bloco_inicio < tamanho) {
            ...
            // Inserir na árvore
            insereArv23(Raiz, Informacao, NULL, NULL, &status);
...
void alocar_memoria_para_programa(Arv23Mem **Raiz){
    ...
    if(*Raiz != NULL){
       ...
       operacao = alocar_memoria((*Raiz), qtd_blocos, (*Raiz)); //primeiro passo, buscar, se encontrar, manipular o bloco e confirmar
...
void desalocar_memoria_sistema(Arv23Mem **Raiz){
   int situacao; 
   ...
   if(*Raiz != NULL){
       ...
       operacao = desalocar_memoria((*Raiz), qtd_blocos, *Raiz); 
